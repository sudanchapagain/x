    ...
    match File::open(format!("server{}", path)) {
        Ok(file) => {
            ...
            if reader.read_to_string(&mut file_content).is_err() {
                return create_response(HttpStatus::InternalServerError, "Internal Server Error".to_string());
            }
            ...
            response.headers.push(("Content-Type".to_string(), content_type.to_string()));
            response
        }
        Err(_) => {
...
fn handle_upload_request(request: &HttpRequest) -> HttpResponse {
   ...
   if let Some(filename) = filename {
        ...
        match std::fs::write(&safe_path, request.body.clone()) {
            Ok(_) => {
                create_response(HttpStatus::Ok, "File uploaded successfully.".to_string())
            },
            Err(_) => {
                create_response(HttpStatus::InternalServerError, "Could not save the file.".to_string())
            }
        }
    } else {
        ...
        Ok(_) => {
            match parse_request(&buffer) {
                ...
                Err(err) => {
                    ...
                    let response = create_response(HttpStatus::BadRequest, format!("Bad Request: {}", err));
                    stream.write_all(response.to_string().as_bytes()).unwrap();
                }
            }
        }
