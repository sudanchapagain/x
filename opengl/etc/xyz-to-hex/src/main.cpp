 ...
 * @param xyz A reference to an array of three floating point values
 * representing the XYZ color space.
 */
...
/**
 * @brief Adapts the XYZ color space from D50 to D65 using a chromatic
 * adaptation matrix.
 *
...
/**
 * @brief Clamps RGB values to the [0, 1] range and scales them to fit within
 * the color gamut.
 * @param rgb A reference to an array of three floating point values
 ...
 * @param adapt If true, performs the adaptation from D50 to D65 (default is
 * true).
 *
 * @return A string representing the RGB color in hexadecimal format (e.g.,
 * "#RRGGBB").
 *
 * @note This function assumes the input color is in CIE1931XYZ format by
 * default. If a different color space is used, the function will return
 * "#000000" as a fallback.
 */
inline std::string cieToHex(float x, float y, float z,
                            const char *type = CIE1931XYZ, bool adapt = true) {
  if (std::strcmp(type, CIE1931XYZ) != 0)
    return "#000000";
  ...
  char buffer[8];
  std::snprintf(buffer, sizeof(buffer), "#%02X%02X%02X", toInt(color[0]),
