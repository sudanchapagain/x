...
use snix_eval::{EvalIO, EvalMode, EvaluationBuilder, Value};

use crate::error::Error;

struct NixDeserializer {
    value: snix_eval::Value,
}
...
/// `snix_eval::Evaluation` provided by the given closure.
...
impl<'de> de::Deserializer<'de> for NixDeserializer {
        ...
        V: de::Visitor<'de>,
    {
        match self.value {
            ...
            // snix-eval types that can not be deserialized through serde.
            Value::Closure(_)
            | Value::Builtin(_)
            | Value::Thunk(_)
            | Value::AttrNotFound
...
struct Enum(snix_eval::NixAttrs);
