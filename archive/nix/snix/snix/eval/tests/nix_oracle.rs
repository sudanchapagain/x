...
/// `NIX_INSTANTIATE_BINARY_PATH` env var to resolve the `nix-instantiate` binary) and snix, and
/// assert that the result is identical
#[track_caller]
#[cfg(feature = "impure")]
fn compare_eval(expr: &str, strictness: Strictness) {
    use snix_eval::{EvalIO, EvalMode};
    ...
    let eval = eval_builder
        .io_handle(Box::new(snix_eval::StdIO) as Box<dyn EvalIO>)
        .build();

    let snix_result = eval
        .evaluate(expr, None)
        .value
        .expect("snix evaluation should succeed")
        .to_string();

    assert_eq!(nix_result.trim(), snix_result);
}

/// Generate a suite of tests which call [`compare_eval`] on expressions, checking that nix and snix
...
// TODO(sterni): snix_tests should gain support for something similar in the future,
