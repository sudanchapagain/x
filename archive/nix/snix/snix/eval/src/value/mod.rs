...
///
/// See `//snix/docs/value-pointer-equality.md` for details.
...
impl Value {
    ...
    async fn deep_force_(myself: Value, co: GenCo, span: Span) -> Result<Option<Value>, ErrorKind> {
        ...
        loop {
            ...
            match value {
                ...
                Value::Thunk(_) => panic!("Snix bug: force_value() returned a thunk"),
                ...
                | Value::FinaliseRequest(_) => panic!(
                    "Snix bug: internal value left on stack: {}",
                    value.type_of()
                ),
            }
        }
    ...
    pub async fn coerce_to_string_(
        ...
        loop {
            ...
            let coerced: Result<BString, _> = match (value, kind) {
                ...
                // C++ Nix and Snix is that the former's strings are arbitrary byte
                // sequences without NUL bytes, whereas Snix only allows valid
    ...
    pub fn explain(&self) -> String {
        match self {
            ...
            | Value::DeferredUpvalue(_)
            | Value::UnresolvedPath(_)
            | Value::FinaliseRequest(_) => "an internal Snix evaluator value".into(),
        }
    }

    /// Constructs a thunk that will be evaluated lazily at runtime. This lets
    /// users of Snix implement their own lazy builtins and so on.
