...
inline void adaptD50toD65(std::array<float, 3>& xyz)
{
    ...
    constexpr std::array<std::array<float, 3>, 3> mInv{{{0.986993f, -0.147054f, 0.159963f},
...
inline std::string cieToHex(float x, float y, float z, const char* type = CIE1931XYZ,
                            bool adapt = true)
{
    ...
    char buffer[8];
    std::snprintf(buffer, sizeof(buffer), "#%02X%02X%02X", toInt(color[0]), toInt(color[1]),
                  toInt(color[2]));
