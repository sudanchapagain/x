...
;;;
;;; Copyright (C) 2010, 2011, 2012, 2020, 2024 Free Software Foundation, Inc.
;;;
...
;; The above license applies to the file as a whole as some code is
;; borrowed from upstream Guile, but the majority of the code falls
;; under the public domain. See below.
...
(define-module (ice-9 match)
  #:pure
  #:use-module (hoot eq)
  #:use-module (hoot equal)
  #:use-module (hoot errors)
  #:use-module (hoot lists)
  #:use-module (hoot not)
  #:use-module (hoot numbers)
  #:use-module ((hoot primitives)
                #:select (%struct? %struct-vtable %struct-ref))
  #:use-module (hoot pairs)
  #:use-module (hoot syntax)
  #:use-module (hoot vectors)
  #:export (match
            match-lambda
            match-lambda*
            match-let
...
(define-syntax-rule (struct? obj) (%struct? obj))
(define-syntax-rule (struct-vtable struct) (%struct-vtable struct))
(define-syntax-rule (struct-ref struct i) (%struct-ref struct i))

;; slot-ref, slot-set!, is-a? borrowed from upstream Guile.
