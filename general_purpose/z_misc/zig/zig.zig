...
// - The type system distinguishes between a pointer to a single value, or
//   multiple values, etc.
// - Slices are preferred, which is a structure with a pointer and a runtime
//   known size, which characterizes most uses of pointers in the first place.
// - Some arbitrary language limitations are removed. For example,
//   enumerations, structures and unions can have functions.
// - Simple access to SIMD operations (basic maths on vectors).
// - Zig provides both low-level features of C and the one provided through
//   compiler extensions. For example: packed structures.
// - An extensive standard library, including data structures and algorithms.
// - Cross-compilation capability is provided by default, without any
//   dependency. Different libc are provided to ease the process.
//   Cross-compilation works from, and to, any operating system and
//   architecture.
...
// Memory isn't managed directly in the standard library, instead an
// "allocator" is asked every time an operation on memory is required. Thus,
// the standard library lets developers handle memory as they need, through
// structures called "allocators", handling all memory operations.

// NOTE: the choice of the allocator isn't in the scope of this document. A
// whole book could be written about it. However, here are some examples,
// to get an idea of what you can expect:

// page_allocator. Allocate a whole page of memory each time we ask for some
// memory. Very simple, very dumb, very wasteful.
// 
// GeneralPurposeAllocator. Get some memory first and manage some buckets of
// memory in order to reduce the number of allocations. A bit complex. Can
// be combined with other allocators. Can detect leaks and provide useful
// information to find them.
// 
// FixedBufferAllocator. Use a fixed buffer to get its memory, don't ask memory
// to the kernel. Very simple, limited and wasteful (can't deallocate), but
// very fast.
// 
// ArenaAllocator. Allow to free all allocated memory at once. To use in
// combinations with another allocator. Very simple way of avoiding leaks.
...
// Namespaces: name conflicts are easily avoided. In practice, that means
// a unified API between different structures (data types).

// Enumerations aren't integers. Comparing an enumeration to an integer
// requires a conversion.

// Explicit casts, coercion exists but is limited. Types are slightly
// more enforced than in C, just a taste: Pointers aren't integers, explicit
// conversion is necessary. You won't lose precision by accident, implicit
// coercions are only authorized in cases where no precision can be lost. /nions
// cannot be reinterpreted (in a union with an integer and a float, one cannot
// take a value for another by accident). Etc.

// Removing most of the C undefined behaviors (UBs), and when the compiler
// encounters one, it stops.

// Slice and Array structures are preferred to pointers. Types enforced by
// the compiler are less prone to errors than pointer manipulations.

// Numerical overflows produce an error, unless explicitly accepted using
// wrapping operators.

// try and catch mechanism. It's both handy, trivially implemented (simple
// error enumeration), and it takes almost no space nor computation time.

// Unused variables are considered to be errors by the compiler.

// Many pointer types exist in order to represent what is pointed to. Example:
// is this a single value or an array, is the length known, etc.

// Structures need a value for their attributes, and it is still possible
// to give an undefined value (stack garbage), but at least it is explicitly
// /ndefined.
