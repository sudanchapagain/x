> [!IMPORTANT]
> I was trying to write 'intro to Nix' but I have been procrastinating for too
> long since I started so i am deferring this for a while. I wanted to publish
> everything when I had at-least written like 70% writing but i don't think i
> am actually ever finishing this so publicly available.

# 0 To X Nix

this is a starter guide for people who want to understand Nix at a high
level with practical outcomes. after this introduction, individuals can
explore Nix on their own. this introduction will not try to be 100% percent
correct semantically but it will try to be more clear and a helpful resource
to newcomers who have a hard time trying understand jargon that is usually
thrown around.

> [!NOTE]
> this comes directly from the notes i took while learning nix myself. i am
> transforming this into a resource so that others like me (nix newbies) can
> atleast understand what nix is at a high level. if something is incorrect
> or can be written in a more coherent manner please raise an issure or if
> possible send a pull request.

> [!WARNING]
> even tho this is being written for newbies there is an implicit assumption
> that you are familiar with basic linux commands, programming/scripting, and
> general computer fundamentals.

## What Is Nix?

Nix is a purely functional package manager. **so, you might wonder what does
purely functional package manager mean?** basically, _purely_ translates to
with same inputs you will always get same outputs and _functional_ translates
to everything happens with functions. **now, what is a function?** function in
nix means a set of transformations described in a block of code. so, purely
functional means always getting same outputs with same inputs with the usage
of functions.

to understand clearly a helpful example would be a calculator. the inputs
are numbers you feed into the calculator and a function is a operation you
want to perform over the numbers (addition, subtraction, etc.). since, the
rule of addition, subtraction, etc does not change over time, you will always
get same the output with same input.

```
╭─────[INPUTS]─────╮
│ - xxxxxxxxxxxxa  │
│ - xxxxxxxxxxxxb  │    ╭────[FUNCTIONS]───╮
│ - xxxxxxxxxxxxc  │    │       ▟▙         │
│ - xxxxxxxxxxxxd  ├────┤       ▟▛▙        │
│ - xxxxxxxxxxxxe  │    │      ▟▛ ▜▙       ├─────╮
╰──────────────────╯    │     ▟▛  ▜▙       │     │
...
package manager is self-explanatory i.e. it manages packages. put everything
together and you get a package manager that makes sures that if you provide
same inputs, it will make sure that you always get the same outputs.


why does this matter?

- re-produce-ability
- docker, ref based package managers

- how does nix guarantee?
  the functional part of nix gurantees.
...
### Using the Nix CLI: Nix-Shell, Nix Develop, Nix Build, Nix Run.

### The Modern CLI (Nix Commands) Vs Legacy (Nix-Shell, Nix-Env).

### Installing Packages Declaratively and Imperatively.
...
## What a Derivation Is and How to Write One.

## How to Package Your Own Project from Source or GitHub.

## Development Shells (DevShells) for Local Projects.
