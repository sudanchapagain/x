...
std::vector<Point> original_polygon
    = { { 150, 150 }, { 300, 450 }, { 450, 350 }, { 350, 100 }, { 200, 50 } };

std::vector<Point> clipped_polygon;

Point
computeIntersection (Point p1, Point p2, int edge)
{
    Point intersect;
    float m;
...
bool
isInside (Point p, int edge)
{
    switch (edge) {
    case 0:
        return p.x >= clip_x_min; // left
    case 1:
        return p.x <= clip_x_max; // right
    case 2:
        return p.y >= clip_y_min; // bottom
    case 3:
        return p.y <= clip_y_max; // tpp
    }
    return false;
}
...
void
drawPolygon (const std::vector<Point>& poly, float r, float g, float b)
{
    glColor3f (r, g, b);
    glBegin (GL_POLYGON);
